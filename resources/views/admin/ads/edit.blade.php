            ...
            <div class="card-body">
                <form action="{{ route('update_ads',$ads->id) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        <input class="form-control @error('name') is-invalid @enderror" type="text" name="name" id="name"  value="{{$ads->name}}">
                    </div>
                    @error('name')
                    ...
                    <div class="form-group ">
                        ...
                        <textarea name="description" class="form-control  @error('description') is-invalid @enderror" id="description" cols="30" rows="10">{{ $ads->description != null ? $ads->description : "" }}</textarea>
                    </div>
                    @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <div class="form-group">
                        ...
                        <div class="thumbnail">
                            ...
                            <div class="preview" id="preview">
                                ...
                                <div class="image-upload"><img src="{{url($ads->thumbnail)}}" alt=""> </div>
                            </div>
                        </div>
                    </div>
                    @error('thumbnail')
                    ...
                    <div class="form-group">
                        ...
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" id="status1" value="pending" {{ $ads->status == 'pending' ? 'checked':"" }}>
                            <label class="form-check-label" for="status1">
                                Chờ duyệt
                            </label>
                        </div>
                        <div class="form-check">
                            <input class="form-check-input" type="radio" name="status" id="status2" value="public" {{ $ads->status == 'public' ? 'checked':"" }}>
