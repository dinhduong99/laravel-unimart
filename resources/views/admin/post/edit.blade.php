                ...
                <form action="{{ route('post.update',$post->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                            <div class="form-group">
                                ...
                                <input class="form-control @error('name') is-invalid @enderror" type="text" name="name" id="name"  value="{{ $post->name }}" data-url="{{ route('slug.product') }}">
                            </div>
                            @error('name')
                            ...
                            <div class="form-group">
                                ...
                                <select name="post_cat_id" class="form-control @error('post_cat_id') is-invalid @enderror">
                                    ...
                                    <option {{ $post->post_cat_id == $cat->id?'selected':''}}  value="{{ $cat->id }}">{{ $cat->name}}</option>
                                    @endforeach
                                </select>
                            </div>
                            @error('post_cat_id')
                    ...
                    <div class="form-group ">
                        ...
                        <textarea name="description" class="form-control  @error('description') is-invalid @enderror" id="description" cols="30" rows="15">{{ $post->description}}</textarea>
                    </div>
                    @error('description')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <div class="form-group">
                        ...
                        <div class="form-check">
                            <input  {{ $post->status == 'pending' ? "checked" :" " }} class="form-check-input" type="radio" name="status" id="status1" value="pending" checked>
                            <label class="form-check-label" for="pending">
                                Chờ duyệt
                            </label>
                        </div>
                        <div class="form-check">
                            <input  {{ $post->status == 'public' ? "checked" :" " }} class="form-check-input" type="radio" name="status" id="public" value="public">
